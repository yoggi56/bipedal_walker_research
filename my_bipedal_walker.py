"""
This is a slighly changed vesion of bipedalWalker-v3 in order to make it 
compatible with PMTG architecture.

### Actions:
The same as in bipedalWalker-v3

### Observations:
The same as using in bipedalWalker-v3.
The only exception is if you use velocity control.
At this situation obs[24] - desired velocity

### Rewards:
The same as in bipedalWalker-v3
The exception is only when you use velocity control.
At this situation reward depends on velocity error, not on maximal robot velocity
...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def __init__(self, learn=True, vel_ctrl=False):
        ...
        self.vel_length = np.random.random_sample()*4 + 1
        self.ref_vel = np.random.random_sample()*0.6
        self.vel_it = 0
        self.learn = learn
    ...
    def _generate_terrain(self, hardcore):
        if self.learn == False:
            GRASS, STUMP1, STAIRS, PIT, _STATES_ = range(5)
        else:
            GRASS, STUMP1, STUMP2, STUMP3, STUMP4, STAIRS, PIT, _STATES_ = range(8)
        ...
        for i in range(TERRAIN_LENGTH):
            if (self.learn == True) and (state == STUMP1 or state == STUMP2 or state == STUMP3 or state == STUMP4):
                state = STUMP1
    ...
    def ref_vel_switch(self):
        if self.vel_it >= self.vel_length:
            self.ref_vel = np.random.random_sample()*0.6
            self.vel_length = np.random.random_sample()*4 + 1
            self.vel_it = 0
    ...
    def step(self, action):
        ...
        # change ref vel if needed
        if self.learn == True and self.vel_ctrl == True:
            self.ref_vel_switch()
        # self.ref_vel = 0.7

        if self.vel_ctrl == True:
            state = [
        ...
        if self.vel_ctrl == True:
            ...
            reward += 4*np.exp(-(self.ref_vel-state[2])**2/0.2)/SCALE
